...
class SecondController extends Controller
{
    ...
    public function actionInsert(){
    	...
    	for($i=0;$i<5000;$i++){
	    	...
	    	$se->community_id	= rand(0,399);
	    	$se->position = '廊坊市' . BaseCommunity::findOne($model->community_id)->name;
			...
			$se->title	= BaseCommunity::findOne($model->community_id)->name . $se->birth . '年房' . $se->price . '万' . $se->area . '平米';
			$se->visit	= 1;
			if(!$se->save()){
				var_dump($se->errors);
				die();
    ...
    public function actionList($d=0, $c=0, $p=0, $a=0, $ro=0, $s=0)
    {
        ...
        $seconds	= $query->offset($pagination->offset)
                    ->limit($pagination->limit)
                    ->all();
		$bestFive = Second::find()->orderBy('id desc')->limit(5)->all();
        return $this->render('list', [
        		'd'		=> $d,	// $district 广阳、安次、开发区
        		'p'		=> $p,	// $price
        		'a'		=> $a,	// $area
        		'ro'	=> $ro,	// $room
        		'searchUrlArray'	=> $searchUrlArray,	// 用于将用post方法提交上来的参数加入到url的get参数列表
        		'search'			=> new Search(),
                'seconds'     		=> $seconds,
                'natural'			=> Second::find()->orderBy('id desc')->limit($pagination->pageSize)->all(),
                'bestFive'			=> $bestFive,
                'condition'			=> $condition,
                'pagination'    	=> $pagination,
            ]);
